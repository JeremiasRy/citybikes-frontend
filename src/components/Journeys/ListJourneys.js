...
export function ListJourneys ({ stations, journeys, setJourneys }) {
  ...
  useEffect(() => {
    async function ParamsChange () {
      ...
      const res = await journeyService.getJourneys(params)
      setJourneys(res)
    } ParamsChange()
  }, [orderBy, orderDirection, pageIndex, pageSize])
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            ...
            <th id='clickable' value='duration' onClick={(event) => { SetOrderParams(event) }}>Duration {
                        orderBy === null
                          ? <><CaretUpFill value='duration' /><CaretDownFill value='duration' /></>
                          : orderBy === 'duration' ? orderDirection === 'a' ? <CaretUpFill value='duration' /> : <CaretDownFill value='duration' /> : <></>
}
            </th>
            <th id='clickable' value='distance' onClick={(event) => { SetOrderParams(event) }}>Distance {
                        orderBy === null
                          ? <><CaretUpFill value='distance' /><CaretDownFill value='distance' /></>
                          : orderBy === 'distance' ? orderDirection === 'a' ? <CaretUpFill value='distance' /> : <CaretDownFill value='distance' /> : <></>
}
            </th>
          </tr>
        </thead>
        <tbody>
          {journeys.map(journey => <tr key={journey.id}>
            ...
            <td>{Math.floor(journey.distance / 1000 * 100) / 100} Km</td>
                                   </tr>)}
        </tbody>
      </Table>
    </div>
