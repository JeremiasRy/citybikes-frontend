...
function App () {
  ...
  useEffect(() => {
    async function InitStations () {
      ...
      const res = await journeyService.getJourneys()
      setJourneys(res)
    } if (stations === undefined) { InitStations() }
    if (journeys === undefined) { InitJourneys() }
  }, [])

  if (stations === undefined || journeys === undefined) { return }
